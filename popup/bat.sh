...
if [ -z $(pgrep -f "n30f.*batgraph\.png") ]; then
  ...
  DURATION="unlimited" BG="#00000000" OVERLAY="true" GEOMETRY="168x10+$(bc <<< "$left + 2")+$(bc <<< "$top + 6")" ./popup.sh "%{c}Charge level is $power."&
  DURATION="unlimited" BG="#00000000" OVERLAY="true" GEOMETRY="168x10+$(bc <<< "$left + 2")+$(bc <<< "$top + 105")" ./popup.sh "%{c} $timetext"&
  sleep 5
  pkill -f "n30f.*bg\.png"
